...
async def login_to_daycomics(page: Page) -> None:
    ...
    try:
        ...
        await page.wait_for_selector('#sideMenuRight > ul > li:nth-child(1) img')
        ...
        await page.wait_for_selector('.sbtLogin')
        await page.click('.sbtLogin')

        # Fill login form
        print(f"{Fore.YELLOW}{Style.BRIGHT}Filling login form")
        await page.wait_for_selector('input[name=email]')
        await page.fill('input[name=email]', os.getenv('DAYCOMICS_LOGIN'))
        await page.click('.emailArea button')
        await page.click('.emailArea button div.absolute div.group:last-of-type div')
        await delay(500)
        await page.fill('input[name=domain]', os.getenv('DAYCOMICS_DOMAIN'))
        await page.fill('input[name=password]', os.getenv('DAYCOMICS_PASSWORD'))
                ...
                for url in urls:
                    ...
                    try:
                        ...
                        episodes = await page.eval_on_selector_all('.episodeListCon a', '''
                            (els, parentTitle) => {
                                return els.map(el => {
                                    ...
                                    let isLockedText = el.querySelector('.coinTagCon button')?.textContent.trim() || 'VIEW';
                                    let isLocked = true;
                                    if (isLockedText.includes('FREE NOW!') || isLockedText.includes('Free for 10 MINUTES')) {
                                        isLocked = false;
                                    }
                        ...
                        for episode in episodes:
                            ...
                            if not episode.get('url') or episode.get('isLocked', True):
                                continue
                            ...
                            # Get images
                            images = await page.eval_on_selector_all('#comicContent div img',
                                                                     'els => els.map(el => el.src.includes("base64") ? el.getAttribute("data-src") : el.src)')
...
if __name__ == "__main__":
    ...
    if args.example:
        ...
        urls = [
            "https://daycomics.com/content/100247",
            "https://daycomics.com/content/102526",
            "https://daycomics.com/content/100106",
            "https://daycomics.com/content/100108",
            "https://daycomics.com/content/102933",
            "https://daycomics.com/content/100250",
            "https://daycomics.com/content/100542",
            "https://daycomics.com/content/100211",
            "https://daycomics.com/content/103060",
            "https://daycomics.com/content/102288",
            "https://daycomics.com/content/100256",
        ]
